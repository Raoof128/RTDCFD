...
class CustomCriticalInfrastructureScenario:
    ...
    def validate_scenario(self) -> Dict[str, Any]:
        ...
        if not self.compliance_requirements.get("soci_act_requirements"):
            validation_results["warnings"].append("Missing SOCI Act compliance requirements")
        
        # Validate custom features
        if self.scenario_name.startswith("custom_"):
            validation_results["recommendations"].append("Consider registering custom scenario in scenarios/__init__.py")
...
def main():
    ...
    for vector in scenario.attack_vectors:
        print(f"  - {vector['name']} (Stage: {vector['attack_stage']}, Impact: {vector['impact']})")
