...
class MCPServer:
    ...
    def __init__(
        self, host: str = "localhost", port: int = 8080, max_connections: int = 50
    ):
        """
    ...
    async def _handle_connection(
        ...
        try:
            ...
            if agent_id in self.connected_agents:
                logger.warning(
                    f"Agent {agent_id} already connected, closing old connection"
                )
    ...
    async def _handle_registration(
        self, agent: ConnectedAgent, message_data: Dict[str, Any]
    ) -> None:
        """
    ...
    async def _route_message(
        self, sender: ConnectedAgent, message: AgentMessage
    ) -> None:
        """
    ...
    async def _send_to_agent(
        self, agent: ConnectedAgent, message: AgentMessage
    ) -> None:
        """
    ...
    async def _send_error_response(
        self, agent: ConnectedAgent, original_message: AgentMessage, error: str
    ) -> None:
        """
    ...
    async def _connection_monitor(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                for agent_id, agent in self.connected_agents.items():
                    if (
                        current_time - agent.last_ping
                    ).seconds > 120:  # 2 minutes timeout
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "server_start_time": (
                self.server_start_time.isoformat() if self.server_start_time else None
            ),
            "uptime_seconds": (
