...
class BlueTeamMCPServer(MCPServer):
    ...
    def update_alert_status(
        self, alert_id: str, new_status: str, assigned_to: str = None
    ) -> None:
        """
        Update alert status.
        ...
        if alert_id in self.active_alerts:
            ...
            self.active_alerts[alert_id]["updated_at"] = str(
    ...
    def trigger_detection_rule(
        self, rule_id: str, trigger_data: Dict[str, Any]
    ) -> None:
        """
        ...
        if rule_id in self.detection_rules:
            ...
            self.detection_rules[rule_id]["last_triggered"] = str(
                asyncio.get_event_loop().time()
            )
...
async def start_blue_team_server(
    host: str = None, port: int = None, max_connections: int = 20
) -> BlueTeamMCPServer:
    """
    Start the blue team MCP server.
