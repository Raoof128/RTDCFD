...
def validate_python_version() -> bool:
    ...
    if version.major < 3 or version.minor < 8:
        logger.error(f"Python {version.major}.{version.minor} not supported. Requires Python 3.8+")
        return False
    
    logger.info(f"✅ Python version validation passed: {version.major}.{version.minor}.{version.micro}")
    return True
...
def print_system_health() -> None:
    ...
    print(f"Configuration: {'✅ OK' if health['checks']['configuration'] else '❌ FAILED'}")
    
    scenarios = health['checks']['scenarios']
    print(f"Scenarios: {scenarios['count']} available ({', '.join(scenarios['available'])})")
...
def validate_mitre_attack_data() -> bool:
    ...
    try:
        ...
        logger.info(f"✅ MITRE ATT&CK data validated: {len(techniques)} techniques loaded")
        return True
