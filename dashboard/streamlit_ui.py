...
from utils.validation import list_available_scenarios, check_system_health
...
def render_header():
    ...
    st.markdown('<h1 class="main-header">🛡️ Autonomous Multi-Agent Simulation Dashboard</h1>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col1:
        st.metric(
            "System Status", 
            "🟢 Operational" if check_system_health()['checks']['configuration'] else "🔴 Issues",
            delta=None
        )
    ...
    with col3:
        st.metric("Simulation Status", 
                 "🟢 Active" if st.session_state.simulation_data['active'] else "🔴 Inactive")
...
def render_sidebar():
    ...
    if not st.session_state.simulation_data['active']:
        ...
        st.sidebar.markdown(f"**Active Scenario:** {st.session_state.simulation_data['scenario']}")
        st.sidebar.markdown(f"**Current Phase:** {st.session_state.simulation_data['current_phase']}")
...
def render_simulation_overview():
    ...
    if not st.session_state.simulation_data['active']:
        st.info("👋 Welcome! Select a scenario from the sidebar and click 'Start Simulation' to begin.")
        return
    ...
    # Progress bar
    phases = ['initialization', 'reconnaissance', 'initial_access', 'execution', 
              'persistence', 'defense_response', 'lateral_movement', 'exfiltration', 'post_incident']
...
def render_timeline(display_options):
    ...
    if not st.session_state.simulation_data['active'] or not display_options['show_timeline']:
        return
    ...
    if attack_timeline:
        ...
        attack_descriptions = [event.get('description', '')[:50] + '...' for event in attack_timeline]
    ...
    if defense_timeline:
        ...
        defense_descriptions = [event.get('description', '')[:50] + '...' for event in defense_timeline]
...
def render_mitre_analysis(display_options):
    ...
    if not st.session_state.simulation_data['active'] or not display_options['show_mitre']:
        return
...
def render_system_logs():
    ...
    log_data = [
        {"timestamp": datetime.now().isoformat(), "level": "INFO", "message": "Simulation started"},
        {"timestamp": datetime.now().isoformat(), "level": "INFO", "message": "Red team agent initialized"},
        {"timestamp": datetime.now().isoformat(), "level": "WARNING", "message": "Unusual network activity detected"},
        {"timestamp": datetime.now().isoformat(), "level": "INFO", "message": "Blue team agent responding"},
        {"timestamp": datetime.now().isoformat(), "level": "ERROR", "message": "Agent communication timeout"},
    ]
...
def start_simulation(scenario_name):
    ...
    st.session_state.simulation_data.update({
        ...
        'agents': {
            'red_recon': {'type': 'reconnaissance', 'status': 'active', 'current_task': 'osint_gathering'},
            'red_social': {'type': 'social_engineering', 'status': 'idle', 'current_task': 'none'},
            'red_exploit': {'type': 'exploitation', 'status': 'idle', 'current_task': 'none'},
            'red_lateral': {'type': 'lateral_movement', 'status': 'idle', 'current_task': 'none'},
            'blue_detection': {'type': 'detection', 'status': 'active', 'current_task': 'monitoring'},
            'blue_response': {'type': 'response', 'status': 'idle', 'current_task': 'none'},
            'blue_intel': {'type': 'threat_intel', 'status': 'idle', 'current_task': 'none'}
        },
        'attack_timeline': [
            {'timestamp': datetime.now().isoformat(), 'description': 'Reconnaissance initiated', 'mitre_technique': 'T1592', 'agent_id': 'red_recon'}
        ],
        'defense_timeline': [
            {'timestamp': datetime.now().isoformat(), 'description': 'Monitoring activated', 'agent_id': 'blue_detection'}
        ],
    ...
    if random.random() < 0.2:  # 20% chance of new event
        ...
        if event_type == 'attack':
            ...
            st.session_state.simulation_data['attack_timeline'].append({
                ...
                'agent_id': random.choice(['red_recon', 'red_social', 'red_exploit', 'red_lateral'])
            })
        else:
            defense_events = [
                'Anomaly detected',
            ...
            st.session_state.simulation_data['defense_timeline'].append({
                ...
                'agent_id': random.choice(['blue_detection', 'blue_response', 'blue_intel'])
            })
