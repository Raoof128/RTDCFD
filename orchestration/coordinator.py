...
from agents import (
    BaseAgent, ReconAgent, SocialEngineeringAgent, 
    ExploitationAgent, LateralMovementAgent,
    DetectionAgent, ResponseAgent, ThreatIntelAgent
)
from mcp_servers import (
    start_main_server, start_red_team_server, start_blue_team_server,
    stop_all_servers
)
from config import settings, ScenarioConfig
...
class SimulationCoordinator:
    ...
    async def _load_scenario(self, scenario_name: str) -> Dict[str, Any]:
        ...
        default_scenarios = {
            "soci_energy_grid": {
                "critical_assets": {
                    "scada_system": {"type": "industrial_control", "criticality": "high"},
                    "power_grid_monitoring": {"type": "monitoring", "criticality": "high"},
                    "employee_portal": {"type": "web_application", "criticality": "medium"},
            ...
            "soci_telco_network": {
                "critical_assets": {
                    "core_network_switches": {"type": "network_infrastructure", "criticality": "high"},
                    "customer_database": {"type": "database", "criticality": "high"},
                    "billing_platform": {"type": "application", "criticality": "medium"},
                    "mobile_network_core": {"type": "telecom_infrastructure", "criticality": "high"}
                },
                "attack_surface": [
            ...
            "soci_water_system": {
                "critical_assets": {
                    "water_treatment_plant": {"type": "industrial_control", "criticality": "high"},
                    "distribution_system": {"type": "infrastructure", "criticality": "high"},
        ...
        return default_scenarios.get(scenario_name, default_scenarios["soci_energy_grid"])
    ...
    async def _initialize_agents(self, scenario_config: Dict[str, Any]) -> None:
        ...
        self.state.agents_active = {agent_id: agent for agent_id, agent in all_agents.items()}
        
        self.logger.info(f"Initialized {len(self.red_team_agents)} red team and {len(self.blue_team_agents)} blue team agents")
    ...
    async def run_simulation(self, timeout_minutes: int = 60) -> None:
        ...
        """
        self.logger.info(f"Starting simulation loop with timeout: {timeout_minutes} minutes")
        
        simulation_end_time = datetime.now() + timedelta(minutes=timeout_minutes)
        
        # Main simulation loop
        while (datetime.now() < simulation_end_time and 
               not self.state.simulation_complete and
    ...
    async def _transition_to_next_phase(self) -> None:
        ...
        if current_index < len(phase_order) - 1:
            ...
            self.logger.info(f"Transitioning from {current_phase.value} to {next_phase.value}")
    ...
    async def generate_report(self) -> str:
        ...
        report_data = {
            ...
            "execution_summary": {
                ...
                "total_duration": str(datetime.now() - self.state.start_time),
                "phases_completed": [phase.value for phase in SimulationPhase if phase != SimulationPhase.COMPLETED]
            },
            "agent_performance": {
                "red_team_agents": {
            ...
            "scoring": {
                "red_team_score": self.state.red_team_score,
                "blue_team_score": self.state.blue_team_score,
                "winner": "red_team" if self.state.red_team_score > self.state.blue_team_score else "blue_team"
            },
            ...
            "critical_assets": {
                ...
                "protected": len(self.state.critical_assets) - len(self.state.compromised_assets)
            },
            "detection_effectiveness": {
                "attacks_detected": len(self.state.detected_attacks),
                "total_attacks": len(self.state.attack_timeline),
                "detection_rate": len(self.state.detected_attacks) / max(len(self.state.attack_timeline), 1)
            }
        }
