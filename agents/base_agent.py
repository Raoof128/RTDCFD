...
Thought: {{agent_scratchpad}}"""
    ...
    async def receive_message(self, message: AgentMessage) -> None:
        ...
        """
        logger.debug(f"Received {message.message_type} message from {message.sender_id}")
    ...
    async def execute_task(self, task: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update metrics
            self.state.metrics["tasks_completed"] = self.state.metrics.get("tasks_completed", 0) + 1
