...
class SocialEngineeringAgent(BaseAgent):
    ...
    def _create_social_engineering_tools(self) -> List:
        ...
        class PhishingContentInput(BaseModel):
            ...
            scenario_type: str = Field(description="Type of phishing scenario (credential, financial, malware)")
        ...
        class PhishingContentTool(BaseTool):
            ...
            def _run(self, target_role: str, scenario_type: str, urgency_level: str) -> str:
        ...
        class PretextingInput(BaseModel):
            ...
            pretext_type: str = Field(description="Type of pretext (IT support, executive, vendor)")
        ...
        class PretextingTool(BaseTool):
            ...
            def _run(self, target_person: str, pretext_type: str, objective: str) -> str:
        ...
        class TargetProfilingInput(BaseModel):
            ...
            profiling_depth: str = Field(description="Depth of profiling (basic, detailed, comprehensive)")
        ...
        if result["success"]:
            ...
            self.narrative_logger.log_attack_event(
                ...
                details={
                    ...
                    "scenario_type": scenario_type,
                    "urgency_level": urgency_level,
                    "content_summary": result["result"][:200] + "..." if len(result["result"]) > 200 else result["result"]
                }
            )
            ...
            self.logger.error(f"Phishing campaign development failed: {result.get('error')}")
    
    async def _handle_pretexting_scenario_command(self, command: Dict[str, Any]) -> None:
        ...
        if result["success"]:
            ...
            self.narrative_logger.log_attack_event(
                ...
                details={
                    ...
                    "pretext_type": pretext_type,
                    "objective": objective,
                    "scenario_summary": result["result"][:200] + "..." if len(result["result"]) > 200 else result["result"]
                }
            )
            ...
            self.logger.error(f"Pretexting scenario development failed: {result.get('error')}")
    ...
    async def _handle_target_profiling_command(self, command: Dict[str, Any]) -> None:
        ...
        task = f"Create {profiling_depth} psychological profile for target: {target_info}"
        result = await self.execute_task(task)
        ...
        if result["success"]:
            ...
            self.narrative_logger.log_attack_event(
                ...
                details={
                    ...
                    "profiling_depth": profiling_depth,
                    "profile_summary": result["result"][:200] + "..." if len(result["result"]) > 200 else result["result"]
                }
            )
